			 ...
			 float *month_inflow)
{
  ...
  for(i=1;i<=ndays;i++) {
    ...
    month_inflow[mm]+=FLOW[i]*CONV_M3S_CM/(DaysInMonth[mm]*years);
  ...
  (*mean_inflow)/=ndays;
   ...
   for (i = 1; i < 13; i++)
     printf("MM[%2d]: inflow=%.1f (m3/day) or %.2f (m3/s)\n",
	   i,month_inflow[i],month_inflow[i]/CONV_M3S_CM);
   printf("mean annual flow = %.1f (m3/day) or %.2f (m3/s)\n\n\n",
     (*mean_inflow), (*mean_inflow) / CONV_M3S_CM);
}
